...
import type { GrowthStage, Planting, ActivityType } from '~/types/garden'
...
// ---- filter tabs (applied before the shared list controls) ----
const filter = ref<'active' | 'ready' | 'all'>('active')
const tabs = computed(() => [{ key: 'active', label: t('plantings.tabActive') }, { key: 'ready', label: t('plantings.tabReady') }, { key: 'all', label: t('plantings.tabAll') }] as const)
...
const inputItems = computed(() => store.inventory.value.map((i) => ({ label: `${i.name} (${store.inventory.value.find((x) => x.id === i.id)?.stock ?? 0} ${t('enums.unit.' + i.unit)})`, value: i.id })))
...
<template>
  <div>
    ...
    <!-- filter tabs -->
    <div class="flex items-center gap-1 p-1 rounded-lg bg-surface border border-app w-fit mb-4">
      <button v-for="tb in tabs" :key="tb.key" class="px-3 py-1.5 rounded-md text-xs font-medium transition" :class="filter === tb.key ? 'bg-surface-2 text-fg' : 'text-muted'" @click="filter = tb.key">{{ tb.label }}</button>
    </div>
    ...
    <UModal v-model:open="actOpen" :title="t('plantings.logActivity')">
      <template #body>
        <div class="space-y-4">
          <UFormField :label="t('plantings.title')">
            <USelect v-model="actForm.plantingId" :items="store.plantings.value.filter((p) => p.stage !== 'harvested').map((p) => ({ label: `${cropById[p.cropId]?.name ?? '—'} · ${plotById[p.plotId]?.name ?? ''}`, value: p.id }))" class="w-full" />
          </UFormField>
          <div class="grid grid-cols-2 gap-4">
      ...
      <template #footer>
        <div class="flex justify-end gap-2 w-full">
          ...
          <button class="btn-accent px-4 py-2 rounded-lg text-sm font-semibold" @click="saveActivity">{{ t('common.save') }}</button>
        </div>
      </template>
    </UModal>
  </div>
